...
INT8U Peripheral_Task( void )
{
	...
	INT8U Buff[PACKET_LENGTH];

	Fill_Memory( &(Buff[0]), PACKET_LENGTH, 0 );
...
*/
	...
	if ( (Adc_X > 135) || (Adc_X < 115) || (Adc_Y > 135) || (Adc_Y < 115) )
	{
		Send_Flag = TRUE;
	}
	...
	if ( Send_Flag == TRUE )
	{
		Buff[ADDR_PACKET_MARK] = PACKET_MARK;
		Buff[ADDR_PACKET_KEY] = Key_Value;
		Buff[ADDR_PACKET_ADCX] = Adc_X;
		Buff[ADDR_PACKET_ADCY] = Adc_Y ;		
		Buff[ADDR_PACKET_CHKSUM] = Chk_Sum( &(Buff[0]), PACKET_LENGTH-1 );
		Rf_T_Data( &(Buff[0]), PACKET_LENGTH );
		//Uart_SendBuff( Buff, PACKET_LENGTH );
		return 1;
	}
	else
	{
